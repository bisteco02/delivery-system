...
let menu = [
    ...
    // Pizzas
    {name: "MIRANDA PRIESTLY (Margherita) - 8 pedaços", category: "pizzas-8", price: 45.00, image: "./assets/Margherita.png", description: "Massa de longa fermentação, molho de tomate, mussarela, parmesão e tomate.", ativo: true},
    {name: "NAZARÉ TEDESCO (Calabresa) - 8 pedaços", category: "pizzas-8", price: 48.00, image: "./assets/Calabresa.png", description: "Massa de longa fermentação, molho de tomate, calabresa, mussarela e parmesão.", ativo: true},
    {name: "DANAERYS TARGERIAN (Frango com requeijão) - 8 pedaços", category: "pizzas-8", price: 50.00, image: "./assets/Frango e Catupiry.jpg", description: "Massa de longa fermentação, molho de tomate, mussarela, frango e requeijão.", ativo: true},
    {name: "CRUELLA (3 queijos) - 8 pedaços", category: "pizzas-8", price: 50.00, image: "./assets/3 queijos.png", description: "Massa de longa fermentação, molho de tomate, mussarela, requeijão e parmesão.", ativo: true},
    {name: "AGATHA TRUNCHBULL (Autoral) - 8 pedaços", category: "pizzas-8", price: 60.00, image: "./assets/agatha.png", description: "Massa de longa fermentação, molho de tomate, mussarela, parmesão, calabresa, carne de sol e requeijão cremoso.", ativo: true},
    {name: "PAOLLA BRACHO (Portuguesa) - 8 pedaços", category: "pizzas-8", price: 55.00, image: "./assets/portuguesa.png", description: "Massa de longa fermentação, molho de tomate, mussarela, parmesão, bacon, requeijão, cebola roxa, tomate e orégano.", ativo: true},
    {name: "MIRANDA PRIESTLY (Margherita) - 4 pedaços", category: "pizzas-4", price: 25.00, image: "./assets/Margherita.png", description: "Massa de longa fermentação, molho de tomate, mussarela, parmesão e tomate.", ativo: true},
    {name: "NAZARÉ TEDESCO (Calabresa) - 4 pedaços", category: "pizzas-4", price: 28.00, image: "./assets/Calabresa.png", description: "Massa de longa fermentação, molho de tomate, calabresa, mussarela e parmesão.", ativo: true},
    {name: "DANAERYS TARGERIAN (Frango com requeijão) - 4 pedaços", category: "pizzas-4", price: 28.00, image: "./assets/Frango e Catupiry.jpg", description: "Massa de longa fermentação, molho de tomate, mussarela, frango e requeijão.", ativo: true},
    {name: "CRUELLA (3 queijos) - 4 pedaços", category: "pizzas-4", price: 30.00, image: "./assets/3 queijos.png", description: "Massa de longa fermentação, molho de tomate, mussarela, requeijão e parmesão.", ativo: true},
    {name: "AGATHA TRUNCHBULL (Autoral) - 4 pedaços", category: "pizzas-4", price: 32.00, image: "./assets/agatha.png", description: "Massa de longa fermentação, molho de tomate, mussarela, parmesão, calabresa, carne de sol e requeijão cremoso.", ativo: true},
    {name: "PAOLLA BRACHO (Portuguesa) - 4 pedaços", category: "pizzas-4", price: 30.00, image: "./assets/portuguesa.png", description: "Massa de longa fermentação, molho de tomate, mussarela, parmesão, bacon, requeijão, cebola roxa, tomate e orégano.", ativo: true},
    ...
    return `<!DOCTYPE html>
        ...
        <head>
            ...
            <style>
                ...
                .item-obs { margin-top: 4px; padding: 3px 5px; border-left: 2px solid #000; background: #efefef; font-size: 12px; font-weight: 700; line-height: 1.35; white-space: normal; word-break: break-word; overflow-wrap: anywhere; }
                .obs { margin-top: 6px; margin-bottom: 6px; padding: 4px 5px; border: 1px dashed #666; background: #fafafa; font-size: 10px; line-height: 1.3; }
                .obs > div:first-child { font-weight: 700; font-size: 10.5px; margin-bottom: 2px; text-transform: uppercase; }
                .obs > div:last-child { font-weight: 500; word-break: break-word; overflow-wrap: anywhere; }
...
// Código de WhatsApp removido - aba não está em uso
...
function iniciarMonitorImpressora() {
    if (intervaloImpressora) {
        console.log('⚠️ Monitor já rodando');
        return;
    }
    
    console.log('🖨️ Monitor de impressão INICIADO');
    
    // Marca pedidos existentes como já processados
    (async () => {
        try {
            const response = await fetchTenant('/pedidos');
...
function pararMonitorImpressora() {
    if (intervaloImpressora) {
        ...
        ultimosPedidosImpressos.clear();
        console.log('❌ Monitor parado');
    }
}

// Setup do toggle
const toggleImpressao = document.getElementById('toggle-impressao-auto');
if (toggleImpressao) {
    // Carregar estado salvo
    const saved = localStorage.getItem('impressaoAutoAtiva');
    impressaoAutoAtiva = saved === 'true';
    toggleImpressao.checked = impressaoAutoAtiva;
    
    if (impressaoAutoAtiva) {
        iniciarMonitorImpressora();
    }
    
    toggleImpressao.addEventListener('change', (e) => {
        impressaoAutoAtiva = e.target.checked;
        localStorage.setItem('impressaoAutoAtiva', impressaoAutoAtiva ? 'true' : 'false');
        
        if (impressaoAutoAtiva) {
            console.log('✅ Impressão automática ATIVADA');
            iniciarMonitorImpressora();
        } else {
            console.log('❌ Impressão automática DESATIVADA');
