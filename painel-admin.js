...
function setTab(tabId, addon = null, index = -1) {
    ...
    function gerarHTMLPedidoParaImpressao(pedido) {
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr class="total-row">
                            ...
                            <td style="text-align: right; font-size: 12px;">R$ ${pedido.total.toFixed(2)}</td>
                        </tr>
                    </tbody>
                </table>
            </div>

            <div style="text-align: center; margin-top: 15px; padding-top: 10px; border-top: 1px solid #000; font-size: 10px;">
    ...
    function imprimirPedidoWeb(pedido) {
        const html = gerarHTMLPedidoParaImpressao(pedido);
        const janela = window.open('', '_blank', 'width=600,height=800');
        janela.document.write(html);
        janela.document.close();
        
        // Aguardar o conteúdo carregar e abrir o dialog de impressão
        janela.onload = function() {
            setTimeout(() => {
                janela.print();
                // Fechar a janela após 1 segundo (deixa o usuário pressionar print)
    ...
    async function carregarPedidos() {
        ...
        if (result.success) {
            ...
            if (novos.length > 0) {
                ...
                // Abrir print dialog para novos pedidos pendentes
                novos.forEach(pedido => {
                    if (pedido.status === 'pendente') {
                        console.log(`🖨️ Abrindo print dialog para pedido #${pedido.numero_pedido || pedido.id}...`);
                        setTimeout(() => {
                            imprimirPedidoWeb(pedido);
                        }, 500);
                    }
                });
